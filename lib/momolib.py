...
def is_valid_desire_address(address, network='mainnet'):
    ...
    try:
        ...
        # rescue from exception, not a valid Momo address
        return False
...
# shims 'til we can fix the momod side
def SHIM_serialise_for_momod(sentinel_hex):
    ...
    # superblock => "trigger" in momod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # momod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    momod_hex = serialise(obj)
    return momod_hex


# shims 'til we can fix the momod side
def SHIM_deserialise_from_momod(momod_hex):
    from models import DESIRED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(momod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('momo.conf')
