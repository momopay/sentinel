...
class MomoConfig():
    ...
    def slurp_config_file(self, filename):
        # read momo.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is momo.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        if args.rpc_port:
            ...
            # standard Momo defaults...
            default_port = 6615 if (network == 'mainnet') else 16615

            # use default port for network if not specified in momo.conf
            if not ('port' in creds):
                creds[u'port'] = default_port

            # convert to an int if taken from momo.conf
