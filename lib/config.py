...
sentinel_cfg = MomoConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_momod_proto_version_with_sentinel_ping = 70207
...
def get_momo_conf():
    ...
    if args.config:
        momo_conf = args.config
    else:
        home = os.environ.get('HOME')
        if home is not None:
            if sys.platform == 'darwin':
                momo_conf = os.path.join(home, "Library/Application Support/MomoCore/momo.conf")
            else:
                momo_conf = os.path.join(home, ".momocore/momo.conf")
        else:
        ...
        momo_conf = sentinel_cfg.get('momo_conf', momo_conf)
