...
def has_momo_conf():
    ...
    valid_momo_conf = False

    # ensure momo_conf exists & readable
    #
    # if not, print a message stating that Momo Core must be installed and
    # configured, including JSONRPC access in momo.conf
    try:
        f = io.open(config.momo_conf)
        valid_momo_conf = True
    except IOError as e:
        print(e)

    return valid_momo_conf
...
def main():
    ...
    if not has_momo_conf():
        print("MomoCore must be installed and configured, including JSONRPC access in momo.conf")
