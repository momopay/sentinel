...
class MomoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some momod methods
    ...
    @classmethod
    def from_momo_conf(self, desire_dot_conf):
        from momo_config import MomoConfig
        config_text = MomoConfig.slurp_config_file(desire_dot_conf)
        creds = MomoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from momolib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = momolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = momolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = momolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_momod_proto_version_with_sentinel_ping)
