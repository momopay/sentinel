...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, momod):
        golist = momod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_momod(momod, item)
    ...
    @classmethod
    def import_gobject_from_momod(self, momod, rec):
        import decimal
        import momolib
        import inflection
        ...
        # shim/momod conversion
        object_hex = momolib.SHIM_deserialise_from_momod(object_hex)
        objects = momolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from momod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from momod, with every run
        ...
        # get/create, then sync payment amounts, etc. from momod - Momod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(momod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from momod! %s" % e)
            govobj.vote_delete(momod)
            return (govobj, None)
    ...
    def vote_delete(self, momod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(momod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, momod, signal, outcome):
        import momolib
        ...
        output = momod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = momolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(momod, signal)

    def sync_network_vote(self, momod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = momod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not momolib.is_valid_desire_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Momo address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (momolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MomoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import momolib
        obj_data = momolib.SHIM_serialise_for_momod(self.serialise())
    ...
    def prepare(self, momod):
        try:
            object_hash = momod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not momolib.is_valid_desire_address(addr, config.network):
    ...
    def hash(self):
        import momolib
        return momolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, momod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - momod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, momod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - momod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, momod):
        now = int(time.time())
        return (self.created_at < (now - momod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, momod):
        if self.is_expired(momod):
            return False

        return True

    def is_deletable(self, momod):
        if self.is_expired(momod):
            return True
