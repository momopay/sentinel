...
def test_valid_desire_address():
    from momolib import is_valid_desire_address
...
def test_hash_function():
    ...
    hex_hash = "%x" % momolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(momolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(momolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(momolib.blocks_to_seconds(16616)) == 2612035
